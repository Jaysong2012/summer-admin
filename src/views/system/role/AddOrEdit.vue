<template>
    <el-dialog v-bind:title="dialogStatus | dialogStatusFilter" :visible.sync="visibale" v-on:close="handleDialogVisible(false)">
        <el-form :model="item" label-width="80px" label-position="left"	>
            ...
            <el-form-item :label="$t('system.role.permission')" v-loading="powerLoading">
                <el-tree
                        class="filter-tree"
                        v-bind:data="data"
                        show-checkbox
                        ...
                        highlight-current
                        :props="defaultProps"
                        v-on:check-change="handleCheckdChange">
                </el-tree>
            </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
            <el-button @click="handleDialogVisible(false)">{{ $t('table.cancel') }}</el-button>
            <el-button type="primary" v-on:click="save">{{ $t('table.confirm') }}</el-button>
        </div>
    </el-dialog>
</template>
    ...
    export default {
        ...
        methods:{
            ...
            save(){
                ...
                saveRole(this.item).then(response => {
